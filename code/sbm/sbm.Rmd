...
```
We might want to visualize the distribution of $\hat{p}_{contra} - \hat{p}_{ipsi}$ under the analytical model:
...
# variances sum
diff.distr.ana = dnorm(diff.distr.emp.mod$x, mean=mean(ips.phat), sd=sqrt(model.var(mean(ips.phat), ne) + model.var(mean(contr.phat), ne)))

n_diff = length(diff.distr.emp.mod$x)
ipsi.dat = data.frame(x = c(ipsi.distr.emp.mod$x, ipsi.distr.emp.mod$x), y = c(ipsi.distr.emp.mod$y, ipsi.distr.ana),
                      distribution=c(rep("empirical", n_ips), rep("analytical", n_ips)))
ipsi.dat$distribution = factor(ipsi.dat$distribution)

ggplot(dat=ipsi.dat, aes(x=x, y=y, ymax=y, fill=distribution, color=distribution, group=distribution)) +
  geom_ribbon(ymin=0, alpha=0.5) +
  ylab('Density') +
  xlab(TeX('$\\hat{p}$')) +
  ggtitle(TeX('Distribution of $\\hat{p}$ ipsi-laterally, DWI'))


contr.distr.ana = dnorm(contr.distr.emp.mod$x, mean=mean(contr.phat), sd=sqrt(model.var(mean(contr.phat), ne)))

n_ips = length(contr.distr.emp.mod$x)
contr.dat = data.frame(x = c(contr.distr.emp.mod$x, contr.distr.emp.mod$x), y = c(contr.distr.emp.mod$y, contr.distr.ana),
                      distribution=c(rep("empirical", n_ips), rep("analytical", n_ips)))
contr.dat$distribution = factor(contr.dat$distribution)

ggplot(dat=contr.dat, aes(x=x, y=y, ymax=y, fill=distribution, color=distribution, group=distribution)) +
  geom_ribbon(ymin=0, alpha=0.5) +
  ylab('Density') +
  xlab(TeX('$\\hat{p}$')) +
  ggtitle(TeX('Distribution of $\\hat{p}$ contra-laterally, DWI'))
```

we can also compare the two empirically and analytically:

```{r}
minx = min(ipsi.distr.emp.mod$x, contr.distr.emp.mod$x)
maxx = max(ipsi.distr.emp.mod$x, contr.distr.emp.mod$x)

n=512
ipsi.distr.emp.cmp = density(ips.phat, from=minx, to=maxx, n = n)
contr.distr.emp.cmp = density(contr.phat, from=minx, to=maxx, n = n)

ipsi.distr.ana.cmp = dnorm(ipsi.distr.emp.cmp$x, mean=mean(ips.phat), sd=sqrt(model.var(mean(ips.phat), ne)))
contr.distr.ana.cmp = dnorm(ipsi.distr.emp.cmp$x, mean=mean(contr.phat), sd=sqrt(model.var(mean(contr.phat), ne)))

emp.dat = data.frame(x = c(ipsi.distr.emp.cmp$x, contr.distr.emp.cmp$x), y=c(ipsi.distr.emp.cmp$y, contr.distr.emp.cmp$y),
                     distribution=c(rep("ipsi", n), rep("contra", n)))


ggplot(dat=emp.dat, aes(x=x, y=y, ymax=y, fill=distribution, color=distribution, group=distribution)) +
  geom_ribbon(ymin=0, alpha=0.5) +
  ylab('Density') +
  xlab(TeX('$\\hat{p}$')) +
  ggtitle(TeX('Distribution of $\\hat{p}_{empirical}$, DWI'))


ana.dat = data.frame(x = c(ipsi.distr.emp.cmp$x, contr.distr.emp.cmp$x), y=c(ipsi.distr.ana.cmp, contr.distr.ana.cmp),
                     distribution=c(rep("ipsi", n), rep("contra", n)))


ggplot(dat=ana.dat, aes(x=x, y=y, ymax=y, fill=distribution, color=distribution, group=distribution)) +
  geom_ribbon(ymin=0, alpha=0.5) +
  ylab('Density') +
  xlab(TeX('$\\hat{p}$')) +
  ggtitle(TeX('Distribution of $\\hat{p}_{analytical}$, DWI'))
```
...
```
We might want to visualize the distribution of $\hat{p}_{contra}$ and $\hat{p}_{ipsi}$ under the analytical model:

```{r}
ne = 1225
ipsi.distr.emp.mod = density(as.numeric(ips.phat))
contr.distr.emp.mod = density(as.numeric(contr.phat))

ipsi.distr.ana = dnorm(ipsi.distr.emp.mod$x, mean=mean(ips.phat), sd=sqrt(model.var(mean(ips.phat), ne)))

n_ips = length(ipsi.distr.emp.mod$x)
ipsi.dat = data.frame(x = c(ipsi.distr.emp.mod$x, ipsi.distr.emp.mod$x), y = c(ipsi.distr.emp.mod$y, ipsi.distr.ana),
                      distribution=c(rep("empirical", n_ips), rep("analytical", n_ips)))
ipsi.dat$distribution = factor(ipsi.dat$distribution)
...
ggplot(dat=ipsi.dat, aes(x=x, y=y, ymax=y, fill=distribution, color=distribution, group=distribution)) +
  ...
  ylab('Density') +
  xlab(TeX('$\\hat{p}$')) +
  ggtitle(TeX('Distribution of $\\hat{p}$ ipsi-laterally, fMRI'))
...
ggplot(dat=contr.dat, aes(x=x, y=y, ymax=y, fill=distribution, color=distribution, group=distribution)) +
  ...
  ylab('Density') +
  xlab(TeX('$\\hat{p}$')) +
  ggtitle(TeX('Distribution of $\\hat{p}$ contra-laterally, fMRI'))
```

the fit is clearly quite good, and we can also compare the two empirically and analytically:
...
ggplot(dat=emp.dat, aes(x=x, y=y, ymax=y, fill=distribution, color=distribution, group=distribution)) +
  ...
  ylab('Density') +
  xlab(TeX('$\\hat{p}$')) +
  ggtitle(TeX('Distribution of $\\hat{p}_{empirical}$, fMRI'))
...
ggplot(dat=ana.dat, aes(x=x, y=y, ymax=y, fill=distribution, color=distribution, group=distribution)) +
  ...
  ylab('Density') +
  xlab(TeX('$\\hat{p}$')) +
  ggtitle(TeX('Distribution of $\\hat{p}_{analytical}$, fMRI'))
```
which clearly shows a strong difference in the means contra-laterally compared to ipsi-laterally. Performing a t-test, we find:
