...
Here, we take each functional and diffusion connectome individually, and compute the parameters of our block model for each connectome. We collect all of the bi-lateral probabilities as our first sample, and the non bi-lateral probabilities as our second sample:
...
graphobj = fmriu.io.collection.open_graphs(basepath = dwi.basepath, atlases = dwi.atlas, datasets = dwi.dsets,
                                           gname = 'ndmg_0-0-48/graphs', fmt='edgelist', rtype = 'array')
...
fmri.basepath = '/data/fmri_edgelists'
...
We might want to visualize the distribution of $\delta = |\hat{p}_{b} - \hat{p}_{n}|$ under the analytical model and compare to our empirical model for functional and diffusion:
...
ggplot(dat=dwi.dat, aes(x=x, y=y, ymax=y, fill=distribution, color=distribution, group=distribution)) +
  ...
  ylab('Density') +
  xlab(TeX('$\\delta_D$')) +
  ggtitle(TeX('Distribution of $\\delta_D = |\\hat{p}_{b} - \\hat{p}_{n}|$, DWI'))
```
...
ggplot(dat=fmri.dat, aes(x=x, y=y, ymax=y, fill=distribution, color=distribution, group=distribution)) +
  ...
  ylab('Density') +
  xlab(TeX('$\\delta_F$')) +
  ggtitle(TeX('Distribution of $\\delta_F = |\\hat{p}_{b} - \\hat{p}_{n}|$, fMRI'))
```
...
ggplot(data=p.dat, aes(x=dataset, y=p, color=dataset, group=dataset)) +
  geom_jitter() +
  coord_trans(y = "log10") +
  ggtitle(TeX(sprintf('Per-subject P-value (2 graphs), %.2f percent have $p < .05$', 100*sum(per.p < .05)/length(per.p)))) +
  xlab('Dataset') +
  ylab('p-value') +
  theme(axis.text.x = element_text(angle=45))
```

As we can see, even at just the $2-$graph level, the difference in bi-lateral vs. non bi-lateral connectivity in the functional vs. diffusion connectomes is significant in most connectomes ($98.99\%$). 
...
Here, we aggregate all of the edge weights bilaterally and non bilaterally, along with the number of edges this information is aggregated over for each connection type. We feed this into a simple t-test with the appropriate assumptions (unequal variance, goal is to test for bilateral connectivity exceeding non bilateral connectivity):
