...
```{r}
ix <- sort(p.failed, index.return=TRUE)$ix[1]
dwfailed = thresh_matrix(dwi.graphs[dwi.failed[ix],,], thresh=0)
fmfailed = fmri.graphs[fmri.failed[ix],,]
...
fmriu.plot.plot_graph(fmfailed, include_diag = TRUE,
                      ...
                      legend.name = "connection", limits=c(0, 2500))
```

```{r}
ix <- sort(p.failed, index.return=TRUE)$ix[2]
dwfailed = thresh_matrix(dwi.graphs[dwi.failed[ix],,], thresh=0)
