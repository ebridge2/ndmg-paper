...
to get the variance term, we note that $\hat{p} = \frac{k}{n}$, so then $Var(\hat{p}) = Var\left(\frac{k}{n}\right) = \frac{1}{n^2} Var(k)$. The binomial theorem can be thought of as an aggregation of $n$ independent bernoulli trials with probability $\hat{p}$; that is, $X_i \overset{iid}{\sim} Bern(p)$ where $\mathbb{E}\left[X_i\right] = p$. Given that the variance of independent events sum, we can expand:
...
\begin{align*}
  ...
  Var(k) &= \sum_{i=1}^n E\left[X_i^2\right] - E\left[X_i\right]^2 \\
  &= np(1-p)
\end{align*}
