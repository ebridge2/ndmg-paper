...
\begin{align*}
    ...
    \end{bmatrix} \\
    \hat{\Sigma}_B &= \begin{bmatrix}
    	\frac{p_{b}(1 - p_{b})}{\left(\frac{N}{2}\right)^2} & 0 \\
    	0 & \frac{p_{n}(1 - p_{n})}{\left(\frac{N}{2}\right)^2}
    \end{bmatrix}
\end{align*}

where $p_b$ represents the probability of a bi-lateral connection, and $p_n$ the probability of a non bi-lateral connection. Then given the connectome megamean as an adjacency matrix $M \in \left\{0, 1\right\}^{N \times N}$ with $N$ vertices, we can compute $p_b$ and $p_n$ as follows:

\begin{align*}
	B &= \left\{(i, j): \textrm{vertex $i$ and vertex $j$ are the same region in opposite hemispheres}\right\} \\
	N &= \left\{(i, j): \textrm{vertex $i$ and vertex $j$ are not the same region in opposite hemispheres}\right\} \\
    p_i &= \frac{1}{|B|} \sum_{(i, j) \in B} M_{ij} \\
    p_c &= \frac{1}{|N|} \sum_{(i, j) \in N} M_{ij}
\end{align*}

Then we have $\delta = |p_b - p_n|$ representing the difference in connectivity from bi-lateral to non bi-lateral. 

## Statistical Goal

With $H_0: p_B <= p_N$, $H_A: p_B > p_N$, determine:

\begin{align*}
  \mathbb{P}(\textrm{reject $H_0$ in favor of $H_A$ | $H_0$ is true})
\end{align*}

That is, determine the probability of incorrectly rejecting the null hypothesis that the connectivity bi-lateral is less than or equal to the connectivity non bi-laterally.
...
```{r}
bi.phat = array(NaN, dim=c(dim(graphs)[1]))
nbi.phat = array(NaN, dim=c(dim(graphs)[1]))
...
for (i in 1:dim(graphs)[1]) {
  ...
  bi.phat[i] = mean(bi)
  nbi.phat[i] = mean(nbi)
  per.p[i] = ana.welch_ttest(bi.phat[i], nbi.phat[i], length(bi), length(nbi), ns1=1, ns2=1, df = 1)$p
}
```

We might want to visualize the distribution of $\delta = |\hat{p}_{n} - \hat{p}_{b}|$ under the analytical model and compare to our empirical model:

```{r}
ne = 1225
diff.distr.emp.mod = density(as.numeric(bi.phat - nbi.phat))

# variances sum
diff.distr.ana = dnorm(diff.distr.emp.mod$x, mean=mean(abs(bi.phat - nbi.phat)), sd=sqrt(model.var(mean(bi.phat), ne) + model.var(mean(nbi.phat), ne)))
...
ggplot(dat=diff.dat, aes(x=x, y=y, ymax=y, fill=distribution, color=distribution, group=distribution)) +
  ...
  ylab('Density') +
  xlab(TeX('$\\delta$')) +
  ggtitle(TeX('Distribution of $\\delta = |\\hat{p}_{b} - \\hat{p}_{n}|$, DWI'))
```
...
As we can see, with just $1$ graph, we are not able to appreciably differentiate bi-lateral from non bi-lateral connectivity. This is likely due to the small number of observations of bi-lateral edges with 1 graph.
...
ggplot(dat=diff.dat, aes(x=x, y=y, ymax=y, fill=distribution, color=distribution, group=distribution)) +
  ...
  ylab('Density') +
  xlab(TeX('$\\delta$')) +
  ggtitle(TeX('Distribution of $\\delta = |\\hat{p}_{b} - \\hat{p}_{n}|$, fMRI'))
```

which clearly shows a strong difference in the means bi-laterally compared to non bi-laterally. Performing a t-test, we find:

```{r}
t.test(bi.phat, nbi.phat, alternative="greater", var.equal=FALSE, paired=TRUE)
```

similar to the diffusion connectomes, the functional connectomes exhibit a difference in bi-lateral vs. non bi-lateral connectivity that is significant with $p < 2.2\times 10^{-16}$. There appears to be significant batch effect in the data, as evinced by the patterns in the $\delta$ empirically.
...
As we can see, with just $1$ graph, we still cannot differentiate bi-lateral from non bi-lateral connectivity, again likely due to the relatively small number of bi-lateral observations relative the non bi-lateral observations.
...
ana.welch_ttest(bi.p, nbi.p, length(bi), length(nbi), ns1=1, ns2=1, df = 1)
```
...
ana.welch_ttest(bi.p, nbi.p, length(dbi), length(dnbi), ns1=1, ns2=1, df = 1)
```

As we can see above, neither the functional nor diffusion connectomes show a significantly appreciable difference in connectivity bi-laterally vs non bi-laterally with just 1 graph, whether that graph is a single subject or modality-specific megamean, likely due to the small number of observations of bi-lateral edges.