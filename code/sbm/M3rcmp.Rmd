...
dwi.per.var = array(NaN, dim=c(dim(fmri.graphs)[1]))
for (i in 1:dim(dwi.graphs)[1]) {
  gr = thresh_matrix(dwi.graphs[i,,])
...
As we can see, even at just the $2-$sample level, the difference in ipsi-lateral vs. contra-lateral connectivity in the diffusion vs. functional connectomes is significant in most connectomes ($99.64\%$). 

##### Example of subject with $p > .05$

```{r}
dwfailed = dwi.graphs[dwi.failed[1],,]
fmfailed = fmri.graphs[fmri.failed[1],,]
fmriu.plot.plot_graph(dwfailed, include_diag = TRUE,
                      title = sprintf("DWI subject %s, delta=%.3f", as.character(dwi.subjects[dwi.failed[1]]), dwi.delta[dwi.failed[1]]),
                      legend.name = "connection", limits = c(min(dwfailed), max(dwfailed)))
fmriu.plot.plot_graph(fmfailed, include_diag = TRUE,
                      title = sprintf("fMRI subject %s, delta=%.3f", as.character(fmri.subjects[fmri.failed[1]]), fmri.delta[fmri.failed[1]]),
                      legend.name = "connection", limits = c(min(fmfailed), max(fmfailed)))
```
...
ana.welch_ttest(ips.p, contr.p, length(fips), length(fcontr), ns1=1, ns2=1, df = 2)
```
...
ana.welch_ttest(ips.p, contr.p, length(dips), length(dcontr), ns1=1, ns2=1, df = 2)
```
